 ...
 * 
 */
package es.uniovi.asw.agents_i3a.web.controller;
...
import es.uniovi.asw.agents_i3a.controller.UserNotFoundException;
 ...
 * @since 08/02/2017
 */
@Controller public class ParticipantsController {
	...
	@GetMapping(value = "/") public String getParticipantInfo( Model model ) {
	...
	@RequestMapping(value = "/userForm", method = RequestMethod.POST) public String showInfo(
			Model model, @ModelAttribute AgentLoginData data ) {
		Agent user = participantsService.getParticipant( data.getLogin(), data.getPassword(),
				data.getKind() );
		if (user == null) {
