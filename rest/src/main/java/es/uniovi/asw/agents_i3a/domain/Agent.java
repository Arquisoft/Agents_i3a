 ...
 * 
 */
package es.uniovi.asw.agents_i3a.domain;
 ...
 * @since 06/02/2017
 */
@Document(collection = "users") @JsonPropertyOrder({ "name", "location", "email", "id", "kind",
		"kindCode" }) public class Agent implements Comparable<Agent> {
	...
	public Agent( String name, String location, String email, String password, String userId,
			int kindCode ) {
	...
	@Override public int compareTo( Agent anotherAgent ) {
	 ...
	 */
	@JsonIgnore @JsonProperty(value = "user_password") public String getPassword() {
		return password;
	}
	 ...
	 * file.
	 * 
	 * @return the kind of user as a plain text. that will depend from a master
	 *         csv file.
	 */
