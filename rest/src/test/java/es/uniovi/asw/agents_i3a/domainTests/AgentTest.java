 ...
 * 
 */
package es.uniovi.asw.agents_i3a.domainTests;
 ...
 * 
 * Adapted by Víctor on 02/02/2018. Modified by Guillermo on 201802041300.
 *
 * @author Damian
...
public class AgentTest {
	...
	@Before public void setUp() throws IOException {
		...
		damian = new Agent( "Damian", "45S30W", "damian@damianmail.com", "damian123", "222222222B",
				3 );
	}
	...
	@Test public void passwordTest() {
		...
		// Setting the password of nico to 1234. And checking the encrypt
		// process works.
	...
	@Test public void identifierTest() {
		...
		// Nico has its id set to null, jorge to 111111111A and damian to
		// 222222222B.
	...
	@Test public void kindTest() throws IOException {
		// The actual CSVFile contains 1,Person;2,Entity;3,Sensor.
		// Setting nico to Person (1); And checking the resolution of the name
		// work
		// properly.
		nico.setKindCode( 1 );
		assertEquals( "Person", nico.getKind() );

		// Nico was previously set to (2) Entity. Checking that both, the code
		// and the
		// resolution work.
		...
		// Changing the value of jorge to a Person and checking now the
		// resolution of
		...
		// Creating a new CSV file with diffetent attributes that the old one.
		// Checking
		// that if the file changes at execution times the resolution of names
		// still
		// works.
		...
		String[] person = { "Person" }, entity = { "Entity" }, sensor = { "Sensor" },
				iphone = { "iPhone" };
		...
		// Now we set nico to 4, a value that was not in the previous master.csv
		// file.
		jorge.setKindCode( 4 );
	...
	@SuppressWarnings("unlikely-arg-type") @Test public void equalsTest() {

		// Creating a copy of nico.
		Agent copyOfNico = new Agent( nico.getName(), nico.getLocation(), nico.getEmail(),
				nico.getPassword(), "111",
				nico.getKindCode() );

		// Another copy of nico.
		Agent anotherCopyOfNico = new Agent( nico.getName(), nico.getLocation(), nico.getEmail(),
				nico.getPassword(),
		...
		Agent nonEqualCopyOfNico = new Agent( nico.getName(), nico.getLocation(), nico.getEmail(),
				nico.getPassword(),
